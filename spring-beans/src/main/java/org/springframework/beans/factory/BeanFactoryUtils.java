 ...
 *在bean工厂上运行的便捷方法，特别是在{@link ListableBeanFactory}接口上。
 ...
 *
 * 返回bean计数，bean名称或bean实例，同时考虑bean工厂的嵌套层次结构（与ListableBeanFactory接口上定义的方法不同，与BeanFactory接口上定义的方法相反）。
 *
 * @author Rod Johnson
 ...
 * @since 04.07.2003
 *
 * BeanFactoryUtils主要就是用来处理ListableBeanFactory清单中的bean的工具，
 * 主要包括处理beanName的前缀来获取原始的beanName
 * 获取ListableBeanFactory和parentFactory (HierarchicalBeanFactory) 获取bean清单和祖父beanFactory中的bean清单
 *
 * 从这个类中可以明白,spring中定义beanName的规则如下：
 * 使用&前缀代表引用
 * 使用#.. 用来区分唯一beanName
 */
public abstract class BeanFactoryUtils {
	 ...
	 *
	 * 生成的bean名称的分隔符。 如果类名或父名不唯一，则将追加“＃1”，“＃2”等，直到名称变为唯一。
	 */
	 ...
	 * 返回实际的bean名称，去掉工厂的dereference前缀（如果有的话，还会删除重复的工厂前缀，如果找到的话）。
	 * @param name the name of the bean
	 * @return the transformed name
	 * @see BeanFactory#FACTORY_BEAN_PREFIX
	 * 意思就是将beanName前面的&去掉，这样返回实际的beanName
	 */
	 ...
	 *
	 * 返回给定名称是否是由默认命名策略（包含“＃...”部分）生成的bean名称。
	 *
	 */
	 ...
	 * 从给定（可能生成的）bean名称中提取“原始”bean名称，不包括可能为唯一性添加的任何“＃...”后缀。
	 ...
	 *
	 * 计算此工厂参与的任何层次结构中的所有bean。 包括祖父Bean工厂的数量。 被“覆盖”的Bean（在具有相同名称的后代工厂中指定）仅计数一次。
	 *
	 ...
	 *
	 * 获取给定类型的所有bean名称，包括在祖先工厂中定义的名称。 在重写bean定义的情况下将返回唯一名称。 考虑FactoryBeans创建的对象，这意味着FactoryBeans将被初始化。
	 * 如果FactoryBean创建的对象不匹配，则原始FactoryBean本身将与该类型匹配。 此版本的{@code beanNamesForTypeIncludingAncestors}自动包含原型和FactoryBeans。
	 *
	 * @param lbf the bean factory
	...
	public static String[] beanNamesForTypeIncludingAncestors(ListableBeanFactory lbf, ResolvableType type) {
		...
		//通过bean清单工厂ListableBeanFactory 获取指定type的所有beanName
		String[] result = lbf.getBeanNamesForType(type);
		//如果lbf是HierarchicalBeanFactory实例，那么获取他的父工厂
		if (lbf instanceof HierarchicalBeanFactory) {
			HierarchicalBeanFactory hbf = (HierarchicalBeanFactory) lbf;
			//判断父工厂是否也是一个bean清单工厂
			if (hbf.getParentBeanFactory() instanceof ListableBeanFactory) {
				//递归的向上一级父工厂中获取所有的beanName
				String[] parentResult = beanNamesForTypeIncludingAncestors(
						(ListableBeanFactory) hbf.getParentBeanFactory(), type);
				//从子工厂中获取了所有的beanName，并合并父工厂的所有的beanName
	 ...
	 * @return the array of matching bean names, or an empty array if none
	 *
	 * 从beanFactory工厂中获取指定类型的beanName
	 *
	 */
	 ...
	 * @throws BeansException if the bean could not be created
	 *
	 * 获取ListableBeanFactory及祖父BeanFactory中的bean
	 *
	 */
	 ...
	 * prototypes and FactoryBeans.
	 *
	 * 返回给定类型或子类型的单个bean，而不是查看祖先工厂。 当我们期望单个bean并且不关心bean名称时，有用的便利方法。
	 * 考虑FactoryBeans创建的对象，这意味着FactoryBeans将被初始化。
	 * 如果FactoryBean创建的对象不匹配，则原始FactoryBean本身将与该类型匹配。 此版本的{@code beanOfType}自动包含原型和FactoryBeans。
	 *
	 * @param lbf the bean factory bean清单工厂
	 * @param type type of bean to match 指定匹配类型
	 * @return the matching bean instance 返回清单工厂中匹配到的类型的bean实例
	 * @throws NoSuchBeanDefinitionException if no bean of the given type was found
	 * @throws NoUniqueBeanDefinitionException if more than one bean of the given type was found
	 * @throws BeansException if the bean could not be created
	 */
	 ...
	 * 从给定的匹配bean Map中提取给定类型的唯一bean。如果不唯一则抛出异常
	 * @param type type of bean to match
